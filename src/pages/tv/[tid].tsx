...
import type { TVShow, Credits, Providers, MovieDB } from "~/types/MovieDB";
...
const TVComponent: NextPage = () => {

    const router = useRouter();
    const { tid } = router.query;

    const { data: results, refetch, isLoading, isError } = useQuery(['tv', tid], async () => {
        ...
        const [mediaRes, creditsRes, similarRes, providersRes] = await Promise.all([
            fetch(`https://api.themoviedb.org/3/tv/${(tid) as string}?api_key=${apiKey}&language=en-US`),
            fetch(`https://api.themoviedb.org/3/tv/${(tid) as string}/credits?api_key=${apiKey}&language=en-US`),
            fetch(`https://api.themoviedb.org/3/tv/${(tid) as string}/recommendations?api_key=${apiKey}&language=en-US`),
            fetch(`https://api.themoviedb.org/3/tv/${(tid) as string}/watch/providers?api_key=${apiKey}`)
        ]);
        const [media, credits, similar, providers] = await Promise.all([
            mediaRes.json() as Promise<TVShow>,
    ...
    if (results.media.name === undefined) {
        return (
